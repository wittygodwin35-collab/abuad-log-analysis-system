...
main() {
    ...
    for dir in "$ROOT_DIR"/*; do
        ...
        # JavaScript/TypeScript mini-services
        ...
        # Python/FastAPI mini-services
        if [ -f "$dir/app.py" ] && [ -f "$dir/requirements.txt" ]; then
            echo ""
            echo "🐍 正在打包 Python 服务: $project_name..."
            ...
            if (
                ...
                tar \
                    ...
                    --exclude='__pycache__' \
                    --exclude='*.pyc' \
                    --exclude='state/*.joblib' \
                    --exclude='state/model_meta.json' \
                    --exclude='state/drain3_state.bin' \
                    -cf - .
            ) | (cd "$output_dir" && tar -xf -); then
                python_bin="python3"
                ...
                if (
                    ...
                    echo "✅ $project_name Python 服务打包成功 -> $output_dir"
                    success_count=$((success_count + 1))
                else
                    echo "❌ $project_name Python 依赖打包失败"
                    fail_count=$((fail_count + 1))
                fi
            else
                echo "❌ $project_name Python 服务复制失败"
                fail_count=$((fail_count + 1))
            fi
        fi
    done
